...
serve(async (req) => {
  ...
  try {
    ...
    if (!formType || !identifier) {
      console.error("Missing required fields:", { formType, identifier });
      return new Response(
    ...
    if (currentCount >= config.maxRequests) {
      console.log(`Rate limit exceeded for ${identifier} on ${formType}`);
    ...
    console.log(`Rate limit check passed for ${identifier} on ${formType}. Remaining: ${remaining}`);
